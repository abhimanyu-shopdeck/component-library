...
- **`ArtifactCard`** — a uniform card for the **Collections** panel (`/artifacts`). Every card is the same structure (fixed-aspect type-specific CSS preview thumbnail — `report`/`document` → spreadsheet mock · `website`/`catalog`/`brand-kit` → storefront mock — + `name` + `time`), so the layout never shifts. **`completed`** cards are clean/passive (no extra UI). **`in-progress`** cards surface a live status pill (white glass + pulsing dot) and a `ProgressBar` **overlaid on the thumbnail** (so height stays identical). Tapping (`onClick`) opens the **preview sheet** (`BottomSheet`: Share + Download `RoundButton` + Continue). The Collections screen has **no filter pills** — calm, scannable, completed-by-default. Entry point: the `SquaresFour` icon in the chat header → `/artifacts`.
