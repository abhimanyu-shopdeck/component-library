...
- **`StarRating`** (`@/components/ui/star-rating`) — interactive 1–5 star rating (gold `warning-amber` filled stars, outline beyond the value). Controlled (`value`) / uncontrolled (`defaultValue`), `onValueChange`, `max` (default 5). For feedback/NPS prompts; use `DataCard type="ratings"` to *display* an existing rating.
