...
- **`DataCard`** (`@/components/ui/data-card`) — data-representation metric surface (Figma `4909:5126`). `type`: `single` (one metric) · `double` (two metrics split by a divider) · `triple` (three centered metrics) · `ratings` (star row). Each `DataMetric` = `{ label, value, cap?, delta?:{value,tone}, progress?:{pct,tone}, icon? }`; `cap` (e.g. "/ ₹5.0 L") shows when there's no `delta`. `delta` = green/red pill; `progress` tones: `orange` (default gradient) | `red` | `brand`. Ratings: `rating`, `outOf`, `ratingLabel`, `ratingNote`.
